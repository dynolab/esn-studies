...
def optimal_esn(training_timeseries,
                test_timeseries_set,
    ...
    best_esn = None
    best_error = np.inf
    errors = np.zeros((len(spectral_radius_values), len(sparsity_values), trial_number))

    for s_r_i, spectral_radius in enumerate(spectral_radius_values):
        print(f'Spectral radius = {spectral_radius}')
        for sp_i, sparsity in enumerate(sparsity_values):
            print(f'\tSparsity = {sparsity}')
            for i in range(random_seed_starts_at, trial_number + random_seed_starts_at):
                print(f'\t\tTrial #{i}')
                esn = ESN(n_inputs=1,
